...
%El equipo de Backend se enfrentó a los siguientes problemas. Se realizó un mal diseño inicial del modelo de datos, en el que se guardaba el estado completo del juego en la Base de Datos, lo que lo hacía muy poco eficiente. En su lugar, se modificó el esquema relacional para almacenar únicamente la información relevante para la API, es decir, para la gestión de las partidas. Este problema estaba relacionado con un planteamiento inicial erróneo de la comunicación con el servidor en el que el envío del estado de la partida en juego y de los eventos se realizaba mediante peticiones HTTP de la misma manera que a la API Rest. Finalmente, por razones obvias de eficiencia se optó por utilizar para este propósito la tecnología de WebSockets. Otro de los problemas encontrados fue el excesivo tiempo dedicado al diseño de la máquina de estados de simulación del juego (aproximadamente mes y medio). Por último, debido a retrasos en el frontend web, se realizó el despliegue de la misma días antes de la entrega final.


\subsubsection*{Equipo android}
El equipo android ha aprendido a utilizar más en profundidad la herramienta de desarrollo android studio, ya que los conocimientos previos que se poseían eran muy básicos. Otro conocimiento adquirido es la conexión con un servidor por medio de JSON Web Token, muy útil a la hora de establecer un canal unidireccional y que además incluye un sistema de firma digital por medio de un token para incrementar la seguridad.
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.75\textwidth]{../images/graficos/GRACIA}
    \caption{Daniel Gracia}
\end{figure}
